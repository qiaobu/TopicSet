package com.raisin.topicset.component.ImageView.handle;

import android.graphics.RectF;
import android.support.annotation.NonNull;

import com.raisin.topicset.component.ImageView.edge.Edge;
...
class CropWindowMoveHelper extends CropWindowScaleHelper {
    ...
    void updateCropWindow(float x, float y, @NonNull RectF imageRect) {
        ...
        if (Edge.LEFT.isOutsideMargin(imageRect)) {
            //获取此时x越界时的坐标位置
            currentCoordinate = Edge.LEFT.getCoordinate();
            //重新指定左边的值为初始值
            Edge.LEFT.initCoordinate(imageRect.left);
            //越界的距离
            offset = Edge.LEFT.getCoordinate() - currentCoordinate;
            //修正最右边的偏移量
            Edge.RIGHT.offset(offset);

        } else if (Edge.RIGHT.isOutsideMargin(imageRect)) {
            currentCoordinate = Edge.RIGHT.getCoordinate();
            Edge.RIGHT.initCoordinate(imageRect.right);
            offset = Edge.RIGHT.getCoordinate() - currentCoordinate;
            Edge.LEFT.offset(offset);

        } else if (Edge.TOP.isOutsideMargin(imageRect)) {
            currentCoordinate = Edge.TOP.getCoordinate();
            Edge.TOP.initCoordinate(imageRect.top);
            offset = Edge.TOP.getCoordinate() - currentCoordinate;
            Edge.BOTTOM.offset(offset);

        } else if (Edge.BOTTOM.isOutsideMargin(imageRect)) {
            currentCoordinate = Edge.BOTTOM.getCoordinate();
            Edge.BOTTOM.initCoordinate(imageRect.bottom);
            offset = Edge.BOTTOM.getCoordinate() - currentCoordinate;
